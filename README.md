...
Claude / Codex 的"今日 ≈成本"都是**按 API 单价估算的等价成本,不是订阅实付**。两者都是订阅制,此数字只反映用量强度(等价 API 账单),实际不按此付费。重度使用时会很大。价格表**外置**在 `pricing.json`(OpenRouter `/api/v1/models` 快照,`python3 usage.30s.py --update-prices` 一键刷新——这是全工具唯一的联网动作);本地特例/别名改 `pricing_overrides.json`(刷新时不被覆盖);未知模型按家族关键字回退、最终回退 opus(偏保守上限)。Codex 按官方分档价(单请求 input ≤272K / >272K 走不同档)逐请求估算。价格会变,按需重跑 `--update-prices` 或改 overrides。
